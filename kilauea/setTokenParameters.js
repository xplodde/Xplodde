...
var address = "0x7b957e8184c9f04cf873ed460fa2cbeef66f3812";
...
console.log(" ");
console.log("setTokenParameters.js");
...
function sendTransaction(tx, gPriced, dX) {
    web3.eth.sendSignedTransaction('0x' + tx.toString('hex'))
        .on('transactionHash', function(receipt) {
            ...
            meta_fwd.methods.setTokenParameters(var1, var2).estimateGas({ gas: 5000000 }, function(error, gasAmount) {
...
meta_xpld.methods.getTiker(forwardAddr).call().then(
    result => {
        console.log(result[0], "Tiker: ", result[1]);
        web3.eth.getTransactionCount(address)
            .then(function(n) {
                var dataX = meta_fwd.methods.setTokenParameters(result[1], 0).encodeABI();
                console.log("dataX: ", dataX);

                var1 = result[1];
                var2 = 0;

                var dataY = meta_xpld.methods.windowTime().call({ from: address }, function(error, result2) {

                    web3.eth.getGasPrice()
                        .then(
                            gasPriced => {
                                // console.log("gasPrice: ", gas);
                                gasPriced = web3.utils.toHex(gasPriced);
                                console.log("gasPrice: ", gasPriced);

                                meta_fwd.methods.setTokenParameters(result[1], 0).estimateGas({ gas: 5000000 }, function(error, gasAmount) {
                                    console.log("gasAmount by estimateGas", gasAmount);
                                    var isNan = isNaN(gasAmount);
                                    ...
                                    setTimeout(function() {
                                        ...
                                        sendTransaction(serializedTx, gasPriced, dataX);
